...
def abbreviate_activities(list_of_activities):
    ...
    for activity in list_of_activities:
        # Split the activity into words
        words = activity.split()
        # Form abbreviation with full first word and first 3 characters of the second word (if available)
        if len(words) > 1:
            abbreviation = words[0] + words[1][:3].capitalize()
        else:
            abbreviation = words[0]  # Use the full single word if there's only one word
        final_word += abbreviation + '-'
    return final_word[:-1]  # Remove the trailing '-'
...
# Example usage:
generate_conformance_heatmap_with_time('Final_Code/running-example.xes',)